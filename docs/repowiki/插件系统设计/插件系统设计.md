...
| 网络 | `fetch(url, opts)` | 原生 Promise + `__go_fetch_async` 后台 goroutine |
| 编码 | `TextEncoder/TextDecoder`、`btoa/atob` | `__go_buffer_from/to_string` + 纯 JS |
| 二进制 | `Buffer.from/alloc/concat/isBuffer` | hex 内部表示 + Go 桥接 |
| 加密 | `crypto.md5/sha256/aesEncrypt/rsaEncrypt/randomBytes` | Go 标准库 `crypto/*` |
...
| `__go_fetch_async` | HTTP 请求 | 真异步（`"fetch:N"`） |
