...
| `SupportedFormats` | 允许的音频扩展名 | `["mp3", "flac", "wav", "ape", "ogg", "m4a", "wma"]` |
...
批量添加电台/广播，歌曲类型为 `TypeRadio`，`is_live = true`。通过 `BatchCreate` 批量入库，不做去重。电台必须提供 `url` 和 `title`。
...
WriteSongTags(filePath, song)
  ...
  (3) tag.WriteTag(filePath, opts) -- 原子写入
        |
        +-- 成功 --> FileWriteWritten
        +-- ErrUnsupportedWrite --> FileWriteUnchanged（M4A/OGG 等不支持的格式）
...
|------|---------|------|
| MP3 | ID3v2.3 | 支持 |
| FLAC | Vorbis Comment + PICTURE | 支持 |
| M4A | -- | 不支持，返回 `ErrUnsupportedWrite` |
| OGG | -- | 不支持，返回 `ErrUnsupportedWrite` |

不支持的格式不阻塞主流程，仅返回 `FileWriteUnchanged` 状态，由调用方决定如何处理。
...
| `/api/v1/settings/scan-auto-create-include-subdirs` | GET/PUT | 自动创建歌单是否包含子目录 |
