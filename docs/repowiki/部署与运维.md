...
**核心原则：底包代表用户意图，只有「同 BUILD_TYPE + 都是 release + data 版本不低于底包」才保留 data 的二进制。**
...
| `dev` | `github.com/.../releases/download/main/version.json` | 开发版（main 分支滚动构建） |

`CheckForUpdates` 同时检查两个渠道，通过 git_commit（优先）或版本号判断是否有更新。
