...
docker build --build-arg VERSION=2.8.0 -t songloft .            # 完整版
...
```bash
make bump              # patch（2.8.0 → 2.8.1）
make bump TYPE=minor   # minor（2.8.0 → 2.9.0）
make bump TYPE=major   # major（2.8.0 → 3.0.0）
```
