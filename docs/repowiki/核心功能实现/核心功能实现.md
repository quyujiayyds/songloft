...
- [internal/app/app.go](file://internal/app/app.go) -- App.Init() 中所有核心组件的创建与装配顺序
- [internal/app/routers.go](file://internal/app/routers.go) -- 路由分组：认证、歌曲、歌单、扫描、缓存、设置、升级等
...
路由分组：音频源编排不直接注册路由端点，而是被 `SongHandler.GetSongPlay`（`/api/v1/songs/{id}/play`）和 `CacheService` 内部调用。`SourceMetrics` 的数据通过 `/api/v1/jsplugins/source-metrics` 暴露给管理界面。
