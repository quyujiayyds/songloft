...
**章节来源**: `lib/config/app_config.dart`、`lib/main.dart`、`lib/core/utils/url_helper.dart`
...
| 平台 | 后端 | 说明 |
|------|------|------|
| Android | ExoPlayer（just_audio 默认） | 原生支持 MP3/FLAC/OGG/M4A/WAV/Opus |
| iOS | AVPlayer（just_audio 默认） | 原生支持 MP3/FLAC/M4A/ALAC/AIFF |
| Web | HTML5 Audio | 支持 MP3/FLAC/OGG/M4A/WAV/Opus |
| Windows/Linux | media_kit（libmpv） | 通过 `just_audio_media_kit` 桥接 |
| macOS | AVPlayer（just_audio 默认） | 同 iOS |
...
使用 Material 3 的 `ColorScheme.fromSeed` 生成完整调色板，seed color 为 indigo-500（`#6366F1`），与后端插件的 CSS 变量体系颜色一致。
...
| 音频后端 | ExoPlayer | AVPlayer | HTML5 Audio | AVPlayer | media_kit | media_kit |
...
| 插件 WebView | InAppWebView | InAppWebView | 新标签页打开 | InAppWebView | InAppWebView | InAppWebView |
...
- Web 平台: 使用桩实现，显示提示文本；实际通过 `launchUrl` 在新标签页打开
...
Web 平台不使用 `flutter_inappwebview`（避免增大构建体积），插件统一通过 `launchUrl` 在浏览器新标签页打开。`PluginWebViewPage` 使用条件导出，Web 构建只包含桩实现。
