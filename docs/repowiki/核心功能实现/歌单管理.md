...
扫描完成后，`SongService` 检查 `scan_auto_create_playlists` 配置（默认 `true`），启用时调用 `runAutoCreatePlaylists`。该方法读取 `scan_auto_create_include_subdirs` 配置（默认 `false`），然后调用 `PlaylistRepository.AutoCreate`。

### 配置开关

| 配置端点 | 配置键 | 默认值 | 说明 |
|---|---|---|---|
| `/settings/scan-auto-create-playlists` | `scan_auto_create_playlists` | `true` | 总开关：扫描后是否自动创建歌单 |
| `/settings/scan-auto-create-include-subdirs` | `scan_auto_create_include_subdirs` | `false` | 子目录开关：是否将子目录歌曲聚合到父目录歌单 |
...
2. **目录聚合**：`include_subdirs=true` 时，每首歌同时加入其所有父目录对应的歌单
